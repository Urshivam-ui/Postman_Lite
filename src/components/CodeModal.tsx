...
import { generateCode, type Language } from '../utils/generateCode';
...
export function CodeModal({ isOpen, onClose }: Props) {
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-3 bg-slate-950/50 border-b border-slate-800">
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-md transition-all cursor-pointer"
          >
            {copied ? (
              <>
